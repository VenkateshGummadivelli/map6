  ...
  const debouncedHandleMessage = useCallback(
    debounce(async (message: string) => {
      ...
      try {
        ...
        const coordinates = coordsMatches?.map((coord: string) => {
        ...
        if ((coordinates?.length ?? 0) >= 2) {
          ...
          if (coordinates) {
            const latitudes = coordinates.map(([lat]: [number, number]) => lat);
            const longitudes = coordinates.map(([, lng]: [number, number]) => lng);
